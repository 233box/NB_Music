class VideoPlayerManager {
        ...
        this.syncInterval = setInterval(() => {
            if (!audioPlayer.audio.paused && !this.videoPlayer.paused) {
                ...
                if (Math.abs(this.videoPlayer.currentTime - audioPlayer.audio.currentTime) > 0.3) {
                    ...
                    const adjustment = currentDiff > 0 ? Math.min(0.05, currentDiff / 10) : Math.max(-0.05, currentDiff / 10);
    ...
    handleVideoLoaded() {
        ...
        // 自动播放，但仅当音频正在播放时
        if (this.playlistManager.audioPlayer && this.playlistManager.audioPlayer.audio && !this.playlistManager.audioPlayer.audio.paused) {
    ...
    async checkCurrentSongHasVideo() {
        try {
            ...
            if (!this.playlistManager || !this.playlistManager.playlist || this.playlistManager.playlist.length === 0 || this.playlistManager.playingNow < 0) {
                return false;
            }
